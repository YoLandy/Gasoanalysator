...
class DataCollector(QObject):
    ...
    def collect_data(self, content, sender):
        if sender.label == "prec":
            self.data_steak["C"]["1"] = int(content["CO2"])
            self.data_steak["C"]["2"] = int(content["CO"])
            self.data_steak["S"]["1"] = int(content["CH"] * 100)

        if sender.label == "rude":
            self.data_steak["C"]["3"] = int(content["CO2"])
            self.data_steak["C"]["4"] = int(content["CO"])
            self.data_steak["S"]["2"] = int(content["CH"] * 100)
